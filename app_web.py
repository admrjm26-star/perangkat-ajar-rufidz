...
def generate_ai(nama_kitab: str) -> PerangkatAjarSalaf:
    system_prompt = (
        ...
        "Tugasmu menyusun Perangkat Ajar (Silabus & RPP) berbasis nama kitab salaf. "
        "Gunakan istilah khas pesantren salaf seperti Bandongan, Sorogan, Lalaran, Mudzakarah, dan Makna Gandul. "
...
def create_docx_bytes(data: PerangkatAjarSalaf) -> BytesIO:
    ...
    r_sub = p_kop.add_run("Sistem Kurikulum & Administrasi Pembelajaran Pesantren Salaf / Diniyah\n")
    ...
    # RPP
    doc.add_heading("III. RENCANA PELAKSANAAN PEMBELAJARAN (RPP) SAMPLE", level=2)
...
# ---------------------------------------------------------
st.markdown("<h1 class='main-title'>PERANGKAT AJAR THE RUFIDZ INDONESIA</h1>", unsafe_allow_html=True)
st.markdown("<p class='sub-title'>Generator Silabus & RPP Otomatis untuk Pesantren Salaf & Madrasah Diniyah</p>", unsafe_allow_html=True)

# Sidebar
st.sidebar.header("📌 Referensi Cepat Kitab")
preset_kitab = st.sidebar.selectbox(
    "Pilih Kitab Populer:",
    ["-- Pilih atau Ketik Manual --", "Safinatun Najah", "Matan Al-Jurumiyah", "Ta'lim Muta'allim", "Fathul Qarib", "Aqidatul Awam", "Taisirul Khollaq"]
)

# Input Nama Kitab
default_val = "" if preset_kitab == "-- Pilih atau Ketik Manual --" else preset_kitab
nama_kitab_input = st.text_input("Nama Kitab Salaf:", value=default_val, placeholder="Contoh: Safinatun Najah / Jurumiyah")
...
if st.button("🚀 Generate Silabus & RPP Otomatis"):
    if not nama_kitab_input.strip():
        ...
        with st.spinner(f"Sedang mengunci bab & menyusun kurikulum untuk '{nama_kitab_input}'..."):
            try:
                # 1. AI Generate
...
if 'data_result' in st.session_state:
    ...
    clean_name = "".join(c for c in data.nama_kitab if c.isalnum() or c in (" ", "_")).rstrip()
